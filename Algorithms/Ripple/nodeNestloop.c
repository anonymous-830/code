 ...
 */
#define MEMORY_MAX 			18809 //Was 10000
...
ExecNestLoop(PlanState *pstate)
{
	...
	unsigned int right_joins = 0;
	unsigned int left_joins = 0;

	// elog(INFO, "Query start");
	...
	{	
					...
					if (TupIsNull(outerTupleSlot))
					{
				 			...
				 			node->js.jointype == JOIN_ANTI))
						{
							...
							if (otherqual == NULL || ExecQual(otherqual, econtext))
							{
								...
								// elog(INFO, "Left tuple: %d has %d joins", node->rippleLeftHead, left_joins);
								left_joins++;
					...
					else
					{
						if (!node->ripple_innerEnd)
						{
								...
								(node->usedMemory < MEMORY_MAX))
							{
								...
								// elog(INFO, "Left cache size = %d and right cache size = %d", node->rippleLeftSize, node->rippleRightSize);
								node->rippleLeftSize++;
								node->usedMemory++;
							}
							/*
				...
				if (node->nl_NeedNewInner && !node->ripple_innerEnd)
				{
					...
					if (TupIsNull(innerTupleSlot))
					{
							 ...
							 node->js.jointype == JOIN_ANTI))
						{
							...
							{
								// elog(INFO, "qualification succeeded, projecting tuple");
								// elog(INFO, "Right tuple: %d has %d joins", node->rippleRightHead, right_joins);
								right_joins++;
								return ExecProject(node->js.ps.ps_ProjInfo);
		...
		if (ExecQual(joinqual, econtext))
		{
			...
			//elog(INFO, "Left tuple: %d matches with %d right tuple", node->rippleLeftHead, node->rippleRightHead);
