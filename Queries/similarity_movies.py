...
def main():
    ...
    for joinQuery, val, dist in joinQueries:
        ...
        data = open(os.path.join(full_path, data_filename + "_lv" + str(dist) + "_Movies" + str(val)), 'w+')
        ...
        for i in range(loop):
            ...
            timeForKs.append(measureTimeForKs(conn, joinQuery, ks, sigma, data_filename+"_Movies", i))
    ...
    for val in vals:
        for dist in lv:
            ...
            summary_unweighted = open(os.path.join(unweighted_path, summary_filename + "_lv" + str(dist) +"_Movies"+ str(val) + "_unweighted.dat"), 'w+')
            summary_weighted = open(os.path.join(weighted_path, summary_filename + "_lv" + str(dist) +"_Movies"+ str(val) + "_weighted.dat"), 'w+')
...
def constructQueries(vals, lv):
    ...
    for val in vals:
        ...
        for dist in lv:
            query = ("SELECT imdb%s.title, omdbMovies%s.title FROM imdb%s JOIN omdbMovies%s ON levenshtein(trim(imdb%s.title::varchar(50)), trim(omdbMovies%s.title::varchar(50))) <= %s LIMIT 3000;" % (val, val, val, val, val, val, dist))
