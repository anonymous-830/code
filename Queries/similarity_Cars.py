...
def main():
    ...
    for joinQuery, val, dist in joinQueries:
        ...
        full_path = "path/to/full/file"

        data = open(os.path.join(full_path, data_filename + "_lv" + str(dist) + "_Cars" + str(val)), 'w+')
        
        # Disable the statement timeout for long-running queries
        cur.execute('set statement_timeout = 1800;')
        
        # Perform the query measurement loop times (currently 1)
        for i in range(loop):
            print("Running this query for the " + str(i) + " time(s)")
            timeForKs.append(measureTimeForKs(conn, joinQuery, ks, sigma, data_filename+"_GRBR", i))
    ...
    for val in vals:
        for dist in lv:
            ...
            unweighted_path = "path/to/unweighted/summaryFile"
            weighted_path = "path/to/weighted/summaryFile"

            summary_unweighted = open(os.path.join(unweighted_path, summary_filename + "_lv" + str(dist) +"_Cars"+ str(val) + "_unweighted.dat"), 'w+')
            summary_weighted = open(os.path.join(weighted_path, summary_filename + "_lv" + str(dist) +"_Cars"+ str(val) + "_weighted.dat"), 'w+')
            ...
            for k in ks:
                ...
                summary_unweighted.write(str(k) + " \t" + str(unweighted_avg) + "\n")
                summary_weighted.write(str(k) + " \t" + str(weighted_avg) + "\n")

    print("Got here!")
    # Close files before exiting
    summary_unweighted.close()
    summary_weighted.close()
    data.close()
    exit()
...
def constructQueries(vals, lv):
    ...
    for val in vals:
        ...
        for dist in lv:
            query = ("SELECT Car_brands%s.make, parking_tickets%s.vehicle_make FROM Car_brands%s JOIN parking_tickets%s ON levenshtein(trim(Car_brands%s.make::varchar(10)), trim(parking_tickets%s.vehicle_make::varchar(10))) <= %s LIMIT 8667;" % (val, val, val, val, val, val, dist))
