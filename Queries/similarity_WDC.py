...
def main():
    ...
    for joinQuery, val, dist in joinQueries:
        ...
        data = open(os.path.join(full_path, data_filename + "_lv" + str(dist) +"_WDC"+ str(val)), 'w+')
        ...
        for i in range(loop):
            ...
            timeForKs.append(measureTimeForKs(conn, joinQuery, ks, sigma, data_filename+"_WDC", i))
    ...
    for val in vals:
        for dist in lv:
            ...
            summary_unweighted = open(os.path.join(unweighted_path, summary_filename + "_lv" + str(dist) +"_WDC"+ str(val) + "_unweighted.dat"), 'w+')
            summary_weighted = open(os.path.join(weighted_path, summary_filename + "_lv" + str(dist) +"_WDC"+ str(val) + "_weighted.dat"), 'w+')
...
def constructQueries(vals, lv):
    ...
    for val in vals:
        for dist in lv:
            query = ("SELECT wdc1Brands{0}.brand, wdc2Brands{0}.brand FROM wdc1Brands{0} JOIN wdc2Brands{0} ON levenshtein(trim(wdc1Brands{0}.brand::varchar(10)), trim(wdc2Brands{0}.brand::varchar(10))) <= {1} LIMIT 3007;".format(val, dist))
